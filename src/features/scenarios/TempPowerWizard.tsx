          ...
          <Card>
            ...
            {results.parallelRunsNeeded && (
              <div className="mt-3 flex items-start gap-2 px-3 py-2 bg-warning/10 border border-warning/30 rounded-lg text-sm text-warning">
                ...
                <span><strong>{fmt(results.ampsPerPhase, 0)}A per phase</strong> — Generator power cable (cam-lock, 50&apos;): 400A/run → {Math.ceil(results.ampsPerPhase / 400)} parallel run{Math.ceil(results.ampsPerPhase / 400) > 1 ? 's' : ''}. Banded 5-wire: 200A/run → {Math.ceil(results.ampsPerPhase / 200)} parallel run{Math.ceil(results.ampsPerPhase / 200) > 1 ? 's' : ''}. Verify cable sizing per NEC tables for your distance and site conditions.</span>
              </div>
            )}
