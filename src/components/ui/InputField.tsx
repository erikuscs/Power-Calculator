  ...
  return (
    <div className="space-y-1.5">
      ...
      <input
        ...
        aria-describedby={error || warning ? messageId : undefined}
        type={type}
        value={value}
      ...
      />
      {error && (
        <p id={messageId} role="alert" className="flex items-center gap-1.5 text-xs text-coral-500">
          <AlertCircle size={12} /> {error}
        </p>
      )}
      {warning && !error && (
        <p id={messageId} className="flex items-center gap-1.5 text-xs text-warning">
